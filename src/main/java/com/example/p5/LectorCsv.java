...
public class LectorCsv {
    ...
    public static ArrayList<Juego> leerArchivo() {
        ...
        String linea;
        String separador = ","; // Cambiar por ";" si tu archivo viene separado por punto y coma
        ...
        try (BufferedReader br = new BufferedReader(new InputStreamReader(is, StandardCharsets.UTF_8))) {
            ...
            // 1. Leer y omitir la línea de encabezados
            ...
            while ((linea = br.readLine()) != null) {
                ...
                // Verificar que la línea tenga las columnas completas para evitar errores
                if (datos.length >= 10) {
                    try {
                        ...
                        String equipo = datos[3].replace("[", "").replace("]", "").trim();
                        ...
                        String generos = datos[7].replace("[", "").replace("]", "").trim();
                        String resumen = datos[8].replace("[", "").replace("]", "").trim();
                        String reviews = datos[9].replace("[", "").replace("]", "").trim();


                        //System.out.println(numero + " : " + nombre);

                        // Crear el objeto y añadirlo a la lista
                        listaVideojuegos.add(new Juego(nombre, fechaLanzamiento, equipo, calificacion, cantidadListado, cantidadReview, generos, resumen, reviews));

                    } catch (NumberFormatException e) {
                        // Saltar filas que tengan datos corruptos en las columnas numéricas
                        continue;
                    }
                }
            }
